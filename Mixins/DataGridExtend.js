...
let DataGridExtend= {
    ...
    * 给外部组件，需求调用，重置表格行宽度
    * */
    setWidthAndHeightForOuter:function(){
    ...
    //表体横行滚动的处理事件
    tableBodyScrollHandler: function (isDetailTable,event) {//监听列表的横向滚动的事件,以便固定表头可以一同滚动  以及纵向滚动的时候,两边固定表格一同滚动
        if(isDetailTable){
    ...
    setAlign:function (type) {
        ...
        for(var i=0;i<realTableBodytr.length;i++){//左右固定栏的行高 要跟真实表格行高保持一致
            ...
            try
            {
                ...
                //fixedLeftTableBodytr[i].children[0].children[0].style.height=(trHeight - 1)+"px";  这个屏蔽，不然会有问题
            }
            catch(err) {}
            ...
            try
            {
                ...
                fixedRightTableBodytr[i].children[0].children[0].style.height=(trHeight - 1)+"px";
            }
            catch(err) {}
        };
            ...
            let filterResult = newHeaders.filter((filterHeader, filterIndex) => {//用户前端代码传的header是否与后端返回的header有重复,返回重复的
                if(filterHeader.name&&filterHeader.label&&(filterHeader.hide!=null&&filterHeader.hide!=undefined)) {
                    if(header.name==filterHeader.name){
                        ...
                        filterHeader.required=header.required;
                        filterHeader.className=header.className;
                        filterHeader.sortAble=header.sortAble;
                        return header;
                    }
                }else{
                    throw new Error("返回的headerData 数据格式不对:{name:'',label:'',hide:true}");
