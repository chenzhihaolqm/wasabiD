        ...
        if(readonly) {//不能直接返回，防止上一次的验证效果还在，导致无法消除
            ...
            {//没有后台验证，或者后台验证已经成功
                if (value!=null&&value!=undefined && value !== "") {//注意一定要加双等号，用户输入了值，验证有效性
                    ...
                    {//多加一层判断，有可能用户
                    ...
                    else if(typeof value==="string"&&(value.indexOf("alert(")>-1||value.indexOf("<script>")>-1)) {//判断有效性，TODO 后期改为正则
                        isvalidate = false;
                        helpTip = "不能填写特殊字符[string、script]";
                    }
                        ...
                        else if (this.props.type=="textrange"){ //范围值输入  
                            let valueArr = value.split(",");  
                            let minVal = valueArr[0],maxVal = valueArr[1];
                            if(minVal !=="" && maxVal !==""){
