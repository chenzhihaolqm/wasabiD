...
let DataGridHandler={
    ...
    *  删除选中的数据
    *  或者
    *  删除了某条的数据而影响到原有的选中数据索引
    * */
    deletecheckedData(delIndex){
        let delKey = this.getKey(delIndex);//获取关键字
        let checkedData=(this.state.checkedData);//已经选中的行
        checkedData.has(delKey) && checkedData.delete(delKey); //先删除数据
        //再把indexKey后面数据的索引递减1
        for (let [key, value] of checkedData.entries()) {
            let [page,index] = key.split("-"); //把数据分成page 跟 index
            if(page == this.state.pageIndex && index>delIndex){
                checkedData.delete(key); //先删除数据
                let newKey = page + "-" + (--index);
                checkedData.set(newKey,value); //再设置值
            }
        }
        this.setState({
            checkedData:checkedData
    ...
    onMouseDown:function(index,event) {//一定要用鼠标按下事件,不保存在状态值中
        if(this.props.focusAble) {
            ...
            this.setState({
                /*这个是2017-7-27添加，设置该状态值原因，需要实时更新渲染，不然会导致有左右固定列的表格，行变化不同步*/
                focusIndex:index
            });   
        }
        this.focusIndex=index;//不更新状态值，否则导致频繁的更新-----2017-7-27前添加
    ...
    *  清除所有选中的数据
    ...
    clearAllCheckedData:function(){
      /*  let checkedData=this.state.checkedData;
        for(let [key,value] of checkedData){
     ...
     *  供外部组件调用触发(获取data数据)
     * */
    getMiniTableDataForOuter(){
       return this.refs.setHeaderTableModal.getDataForOuter();
    },

    /*
     *  供外部组件调用触发(更新data数据)
     * */
    setMiniTableDataForOuter(updata){
        if(updata instanceof Array){
            this.refs.setHeaderTableModal.setDataForOuter(updata);
        }
    },
    resetHandler(){
