...
let  Import=React.createClass({
    propTypes: {
        ...
        startRow: React.PropTypes.number,//读取开始的行数，如果只有一行标题的话，默认是2，如果标题不止一行就要传入该值
        title:React.PropTypes.string,   //标题
        tipMessage:React.PropTypes.string,   //弹出框的提示语
    },
    ...
    componentWillReceiveProps: function (nextProps) {
        ...
        this.setState({
            ...
            tipMessage:nextProps.tipMessage,
    ...
    uploadComplete:function (index,event) {
        ...
        if ( xhr.readyState == 4 && xhr.status == 200 ) {
            ...
            if(result&&result.success!=null&&result.success!=undefined) {
                if(result.success==true) {
                    ...
                    {//导入文件成功
                        ...
                        let  fileId = result.data.fileId; // 文件Id
                        ...
                        let  startRow = this.props.startRow;
                        if (result.data.success) {
                            this.successNum+=1;
                            uploadInfo.unshift(<div className="success" key={"success"+(index+startRow).toString()}>{"序号为" + (index + startRow).toString() + "行,导入成功"}</div>)
                        }
                        else {
                            this.failNum+=1;
                            uploadInfo.unshift(
                                <div className="fail" key={"fail"+(index+startRow).toString()}>{"序号为" + (index + startRow).toString() + "行,导入失败," + result.data.message}</div>)
                        }
                        this.setState({
                    ...
                    if(index==null)
                    {
                        ...
                        if(index>=this.total-1) {//代表已经执行完最后一条记录了
                            ...
                            {
                                this.props.importSuccess(result,this.state.fileId);
                            }
            ...
            this.clearFile();
            let errorMsgHook={
                "404":"服务器没有响应,请检查您的上传路径",
                "413":"上传文件过大",
                 "-1":"服务器没有响应,请检查您的上传路径"
            }
            let errorMsgStr = errorMsgHook[xhr.status]?errorMsgHook[xhr.status]:errorMsgHook["-1"];
            Message.error(errorMsgStr);
            this.setState({
    ...
    render:function() {
        ...
        let title = this.state.title; //请选择导入文件
        let failloadurl = this.props.failloadurl + "&fileId="+this.state.fileId;
        return(   <Modal  ref="modal"  width={460} height={340} title={title}>

            <div className="import-tip-message" style={{display:this.state.tipMessage!=""?"block":"none"}}>
                <div> {this.state.tipMessage}</div>
            </div>
            ...
            <div className="import-submit">
                <a className="import-failload" target="blank" href={failloadurl} style={{display:this.state.showfail==true?"inline":"none"}}>下载失败信息</a>
