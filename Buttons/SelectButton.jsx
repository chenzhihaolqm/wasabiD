...
let  SelectButton  = React.createClass({
    ...
    propTypes: {
        ...
        trigger:React.PropTypes.string,//触发方式 'click'、'hover'
        ...
        onSelect:React.PropTypes.func, // 子选项选择
        saveActive:React.PropTypes.bool,//是否保存选择过的状态样式
    },
    ...
    onMouseEnter:function(name,title,event){
        if(this.props.trigger !== "hover" || this.state.disabled ){
            return;
        }
        this.setState({
            hide:false,
    ...
    onMouseLeave:function(name,title,event){  
        if(this.props.trigger !== "hover" || this.state.disabled ){
            return;
        }
        this.setState({
    ...
    onSelect(rowData,event){//下拉项  点击事件
        ...
        this.props.onSelect&&this.props.onSelect(this.props.name,this.state.title,event,rowData.value,rowData.text,rowData);
        this.setState({
            hide:true,
            value:rowData.value,//将最后选中的项  标出来
        });
    },
    render(){
        var title=<span>{this.state.title}<i className="icon-drop"></i></span>
        return (
            <div className="wasabiD-selectButton" onMouseEnter={this.onMouseEnter}
                ...
                <ul className="selectPanel" ref="selectPanel" style={{display:this.state.hide?"none":"block"}}>
                    {
                        this.state.data.map((child, i)=> {
                            let liCls = this.props.saveActive?(child.value == this.state.value ? "active" : ""):"";
                            if(child.hasBottomLine){
                                liCls += " bottom-line";
                            }
                            return (
                                <li key={"li" + i} className={liCls }
                                    onClick={this.onSelect.bind(this, child)}>{child.text}</li>
                            )
                        })
                    }
                </ul>
