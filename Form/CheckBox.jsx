...
let CheckBox=React.createClass({
    ...
    PropTypes:{
        ...
        stopPropagationFlag:React.PropTypes.bool,//点击文本，是否停止向上冒泡
    ...
    stopPropagationFn:function(e){
        if(this.props.stopPropagationFlag){
            e.stopPropagation();
        }
    },
...
e.preventDefault();//因为有用户借助label属性生成新的checkbox,所以要阻止默认事件
        ...
        let index = oldvalue.indexOf(value.toString());
        if (index> -1) {//取消选中
        ...
        if(this.state.data instanceof  Array) {
            control= this.state.data.map((child, i)=> {
                ...
                var rowProps={
                    ...
                    readOnly:!!this.state.readonly,  //原生标签要求readOnly
                }
                var props={
                    checked:(checked==true?"checked":null),//是否为选中状态
                    readonly:!!this.state.readonly,//组件要求传readonly
                }
                 ...
                 return  (
                     <li key={i} onClick={this.onSelect.bind(this,child.value,child.text,child)}  >
                         <input type="checkbox"  id={"checkbox"+this.props.name+child.value}  value={child.value}
                                onChange={this.changeHandler} className="checkbox"  {...rowProps}></input>
                         <label className={"checkbox-label "+(rowProps.readOnly?"readonly":"")}></label>
                         <div  className="checktext" onClick={this.stopPropagationFn}  >{child.text}</div>
                     </li >   
                 )
