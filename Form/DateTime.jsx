...
let DateTime = React.createClass({
    ...
    render:function(){
        ...
        if(this.state.time){
            ...
            try{
                timeParam.hour = Number(timeArr[0]);
                timeParam.minute = Number(timeArr[1]);
                timeParam.second = Number(timeArr[2]);
            }catch(e){
                console.log(e);
            }
        }  
        return(
            <div>
            <div style={{position:"relative",height:32}}><Time ref="time" type="time"  key="end" {...timeParam} ></Time></div>
